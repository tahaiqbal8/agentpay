...
**Status: implemented in the gateway, NOT yet executed. Program v2 is not
deployed. No session has ever run under it on devnet.**

| | |
| --- | --- |
| v1 program (live) | `3aKGM6Cb4Rd5sPH5YmSFc9567xNCDDKschQ4u7y5xP2U` |
| v2 program (built, undeployed) | `ApjxJKBUUd8EEAovQe74jS9qZsRCAC2bwe8hTx7TpS9m` |
...
1. **Deploy v2.** v1 keeps serving every existing session. Nothing changes for
   anyone.
2. **Point new sessions at v2.** Agents read `settlement_authority` from
   `/health` and bind it at `open_session`. Until this step, v2 is deployed and
   idle.
3. **Stop opening v1 sessions.** The cutover point. After it, the v1 program is
   read-and-drain only.
4. **Drain.** Sessions are expiry-bounded — `MAX_SESSION_DURATION_SECS` is 30
...
5. **Retire.** When no v1 session has a non-zero vault, remove
...
Neither program has been audited. v2 has not been deployed. Nothing in this
document has been verified on devnet — see the status line at the top.
